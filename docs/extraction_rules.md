...
### 7.1 SymInt Example Value 必须正确推断
...
**原因**：`_assert_scalar` 期望 Python bool，但 `item <= s0` 返回 Tensor
...
| ✅ 可运行性 | 必须能执行 forward | 运行 `python3.10 -m graph_net.torch.run_model` |
